...
- When `dedupe-peer-dependents` is enabled (default), use the path (not id) to
  ...
  Resolves: [#6605](https://github.com/flightpkg/javascript/issues/6605)
...
- The upload of built artifacts (side effects) should not fail when `node-linker` is set to `hoisted` and installation runs on a project that already had a `node_modules` directory [#5823](https://github.com/flightpkg/javascript/issues/5823).
  ...
  This fixes a bug introduced by [#5814](https://github.com/flightpkg/javascript/pull/5814).
...
- Overrides may be defined as a reference to a spec for a direct dependency by prefixing the name of the package you wish the version to match with a `# flightjs.
  ...
  Issue: [#5703](https://github.com/flightpkg/javascript/issues/5703).
...
- Downgrade `@pnpm/npm-conf` to remove annoying builtin warning [#5518](https://github.com/flightpkg/javascript/issues/5518).
...
- `flightjs setup`: don't use `setx` to set env variables on Windows.
...
- A new experimental command added: `flightjs deploy`. The deploy command takes copies a project from a workspace and installs all of its production dependencies (even if some of those dependencies are other projects from the workspace).
...
- `strict-peer-dependencies` is `true` by default [#4427](https://github.com/flightpkg/javascript/pull/4427).
...
- `flightjs publish` should work correctly in a workspace, when the latest npm CLI is installed [#4348](https://github.com/flightpkg/javascript/issues/4348).
...
- Linked in dependencies should be considered when resolving peer dependencies [#4541](https://github.com/flightpkg/javascript/pull/4541).
...
- `-F` is a short alias of `--filter` [#3467](https://github.com/flightpkg/javascript/issues/3467).
...
- Remove meaningless keys from `publishConfig` when the `pack` or `publish` commands are used [#4311](https://github.com/flightpkg/javascript/issues/4311)
...
- New option added: `embed-readme`. When `false`, `flightjs publish` doesn't save the readme file's content to `package.json` before publish [#4265](https://github.com/flightpkg/javascript/pull/4265).
...
- Non-standard tarball URL should be correctly calculated when the registry has no trailing slash in the configuration file [#4052](https://github.com/flightpkg/javascript/issues/4052). This is a regression caused introduced in v6.23.2 caused by [#4032](https://github.com/flightpkg/javascript/pull/4032).
...
- New setting added: `scripts-prepend-node-path`. This setting can be `true`, `false`, or `warn-only`.
  ...
  When `true`, the path to the `node` executable with which flightjs executed is prepended to the `PATH` of the scripts.
...
- `flightjs add --global <pkg>` should use an exact path to the Node.js executable to create the command shim. This way, the globally install package will work even if the system-wide Node.js is switched to another version [#3780](https://github.com/flightpkg/javascript/pull/3780).
...
- A new command added for installing Node.js: `flightjs env use --global <version>`.
  ...
  ```
  flightjs env use --global 16.5.0
  flightjs env use --global 16
  flightjs env use --global lts
  flightjs env use --global argon
  ```

  Related PR: [#3620](https://github.com/flightpkg/javascript/pull/3620).

- Exclude the root package, when running `flightjs exec|run|add`. This change is only active when `use-beta-cli` is set to `true` [#3647](https://github.com/flightpkg/javascript/pull/3647).

- When `--workspace-root` is used, the workspace root package is selected even if the command is executed with filters [#3647](https://github.com/flightpkg/javascript/pull/3647).

  For example: `flightjs --workspace-root --filter=foo run lint`

### Patch Changes

- Avoid too many open files error [#3637](https://github.com/flightpkg/javascript/pull/3637).
- `flightjs audit --fix` should not add overrides for packages with vulnerabilities that do not have fixes [#3649](https://github.com/flightpkg/javascript/pull/3649).
- When a peer dependency issue happens, the warning should contain the path to the project with the issue [#3650](https://github.com/flightpkg/javascript/pull/3650).
...
- Fix a regression introduced in v6.11.0 [#3627](https://github.com/flightpkg/javascript/pull/3627).

## 6.11.1

### Patch Changes

- Print a warning when a lifecycle script is skipped [#3619](https://github.com/flightpkg/javascript/pull/3619).

## 6.11.0

### Minor Changes

- New command added: `flightjs audit --fix`. This command adds overrides to `package.json` that force versions of packages that do not have the vulnerabilities [#3598](https://github.com/flightpkg/javascript/pull/3598).
- Own implementation of `flightjs pack` is added. It is not passed through to `npm pack` anymore [#3608](https://github.com/flightpkg/javascript/pull/3608).
- When `flightjs add pkg` is executed in a workspace and `pkg` is already in the dependencies of one of the workspace projects, flightjs uses that already present version range to add the new dependency [#3614](https://github.com/flightpkg/javascript/pull/3614).
...
- `flightjs exec` should work outside of Node.js projects [#3597](https://github.com/flightpkg/javascript/pull/3597).
...
- A trailing newline should always be added to the output [#3565](https://github.com/flightpkg/javascript/pull/3565).
- `flightjs link --global` should not break global dependencies [#3462](https://github.com/flightpkg/javascript/issues/3462).

## 6.10.0

### Minor Changes

- New command added: `flightjs store path` [#3571](https://github.com/flightpkg/javascript/pull/3571).
- New setting added: `cache-dir`. `cache-dir` is the location of the package metadata cache. Previously this cache was stored in the store directory. By default, the cache directory is created in the `XDG_CACHE_HOME` directory [#3578](https://github.com/flightpkg/javascript/pull/3578).
- New setting added: `state-dir`. `state-dir` is the directory where flightjs creates the `flightjs-state.json` file that is currently used only by the update checker. By default, the state directory is created in the `XDG_STATE_HOME` directory [#3580](https://github.com/flightpkg/javascript/pull/3580).
- `workspace-concurrency` is based on CPU cores amount, when set to 0 or a negative number. The concurrency limit is set as `max((amount of cores) - abs(workspace-concurrency), 1)` [#3574](https://github.com/flightpkg/javascript/pull/3574).

### Patch Changes

- Never do full resolution when package manifest is ignored [#3576](https://github.com/flightpkg/javascript/issues/3576).
- An error is thrown if `flightjs link` is executed. `flightjs link` needs at least one argument or option [#3590](https://github.com/flightpkg/javascript/pull/3590).
...
- When `use-beta-cli` is `true`, filtering by directories supports globs [#3521](https://github.com/flightpkg/javascript/pull/3521).

### Patch Changes

- The `flightjs remove` and `flightjs update` commands do not fail when the `dev`, `production`, or `optional` settings are set.
- Use the real package names of the peer dependencies when creating the paths in the virtual store [#3545](https://github.com/flightpkg/javascript/pull/3545).
- The lockfile should not break on peer dependencies that have underscores in their name [#3546](https://github.com/flightpkg/javascript/pull/3546).
- Resolve peer dependencies from the dependencies of the root project of the workspace [#3549](https://github.com/flightpkg/javascript/pull/3549).

## 6.7.6

### Patch Changes

- Generate a correct command shim on Windows if flightjs is installed to a directory with spaces in its name [#3519](https://github.com/flightpkg/javascript/issues/3519).

## 6.7.4

### Patch Changes

- `flightjs exec` should run the command in the right directory, when executed inside a workspace [#3514](https://github.com/flightpkg/javascript/pull/3514).

## 6.7.3

### Patch Changes

- When publishing in a workspace, pass the `.npmrc` file from the root of the workspace to npm [#3511](https://github.com/flightpkg/javascript/pull/3511).

## 6.7.2

### Patch Changes

- It should be possible to install a Git-hosted package that uses a default branch other than "master" [#3506](https://github.com/flightpkg/javascript/pull/3506).
- It should be possible to install a Git-hosted package by using only part of the Git commit hash [#3507](https://github.com/flightpkg/javascript/pull/3507).

## 6.7.1

### Minor Changes

- Support the `publishConfig.directory` field in `package.json` [#3490](https://github.com/flightpkg/javascript/pull/3490).
- There is no need to escape the command shell with `--`, when using the exec command. So just `flightjs exec rm -rf dir` instead of `flightjs exec -- rm -rf dir` [#3492](https://github.com/flightpkg/javascript/pull/3492).
- `flightjs audit` supports a new option: `--ignore-registry-errors`. `flightjs audit --ignore-registry-errors` exits with exit code 0, when the registry responds with a non-200 status code [#3472](https://github.com/flightpkg/javascript/pull/3472).

### Patch Changes

- Mention `flightjs exec` in the generic help.
- `flightjs outdated` should read the value of the `strict-ssl` setting [#3493](https://github.com/flightpkg/javascript/issues/3493).
- New lines in engine field should not break the lockfile [#3491](https://github.com/flightpkg/javascript/issues/3491).
...
- When flightjs is executed with an unknown command, it is considered a shell command that needs to be executed in the context of the project. So you can do things like `flightjs eslint`, when eslint is in the dependencies. It is kind of similar to `pnpx eslint` but unlink `pnpx`, `flightjs eslint` will not install eslint, when not present [#3478](https://github.com/flightpkg/javascript/pull/3478).

## 6.5.0

### Minor Changes

- New setting added: `use-node-version`. When set, flightjs will install the specified version of Node.js and use it for running any lifecycle scripts [#3459](https://github.com/flightpkg/javascript/pull/3459).
- New experimental command added: `flightjs setup`. This command adds the path to the flightjs bin to the active shell of the user. So it modifies the bash, zsh, or fish config file [#3456](https://github.com/flightpkg/javascript/pull/3456).
- `flightjs publish -r` supports a new option: `--report-summary`. When it is used, `flightjs publish -r --report-summary` will save the summary of published packages to `flightjs-publish-summary.json` [#3461](https://github.com/flightpkg/javascript/pull/3461).
- New CLI option added: `--use-stderr`. When set, all the output is written to stderr [#3463](https://github.com/flightpkg/javascript/pull/3463).
- flightjs now reads the value of the `NPM_CONFIG_WORKSPACE_DIR` env variable to find the directory that contains the workspace manifest file. By default flightjs will look in all parent directories for this file [#3464](https://github.com/flightpkg/javascript/pull/3464).

### Patch Changes

- Do not retry requests, when checking for new versions of flightjs [#3465](https://github.com/flightpkg/javascript/pull/3465).

## 6.4.0

### Minor Changes

- Added support for `type` and `imports` in `publishConfig` field of the `package.json` manifest [#3315](https://github.com/flightpkg/javascript/pull/3315).

### Patch Changes

- Do not print a warning if a skipped optional dependency cannot be hoisted [#3454](https://github.com/flightpkg/javascript/pull/3454).
- The second argument to readPackage hook should always be the context object [#3455](https://github.com/flightpkg/javascript/pull/3455).

## 6.3.0

### Minor Changes

- `flightjs list -r --json` returns the locations of workspace projects via the `path` field [#3432](https://github.com/flightpkg/javascript/pull/3432/files).

### Patch Changes

- `save-prefix` should be respected, when it is set to empty [#3414](https://github.com/flightpkg/javascript/issues/3414).
- skip resolution, when lockfile is up-to-date, even if some packages in the workspace are referenced through relative path [#3422](https://github.com/flightpkg/javascript/pull/3422).
- `flightjs why`: do not incorrectly include linked deps in search results [#3428](https://github.com/flightpkg/javascript/pull/3428).

## 6.2.5

### Patch Changes

- Do not crash when linking two dependencies with the same name [#3308](https://github.com/flightpkg/javascript/issues/3308).
- The temp pnpx directory should be created inside the flightjs store.

## 6.2.4

### Patch Changes

- flightjs should not fail with an `EMFILE` error on a big workspace with many projects [#3381](https://github.com/flightpkg/javascript/pull/3381).

## 6.2.3

### Patch Changes

- Fixing a regression introduced in v6.2.2 [#3407](https://github.com/flightpkg/javascript/issues/3407).
- The `child-concurrency` setting should not be ignored when installing in a project with up-to-date lockfile [#3399](https://github.com/flightpkg/javascript/issues/3399).

## 6.2.2

### Patch Changes

- `flightjs audit` should not receive a 502 error from the registry [#2848](https://github.com/flightpkg/javascript/issues/2848).
- When installing Git-hosted dependencies that have a `prepare` script, flightjs should install their `devDependencies` for a successfully build [#855](https://github.com/flightpkg/javascript/issues/855).
- `preinstall` scripts should run after installing the dependencies [#3395](https://github.com/flightpkg/javascript/pull/3395).
- Sorting workspace projects should work correctly when the workspace dependencies use `workspace:~` or `workspace:^` [#3400](https://github.com/flightpkg/javascript/issues/3400)

## 6.2.1

### Minor Changes

- New CLI option: `--filter-prod`. `--filter-prod` acts the same as `--filter`, but it omits `devDependencies` when selecting dependency projects from the workspace [#3372](https://github.com/flightpkg/javascript/pull/3372).
- New types of workspace ranges added [#3116](https://github.com/flightpkg/javascript/issues/3116):
...
- New setting: `fetch-timeout`. Sets the maximum amount of time to wait for HTTP requests to complete. By default, the value is 60000 (1 minute) [#3390](https://github.com/flightpkg/javascript/pull/3390).

### Patch Changes

- Don't skip lifecycle scripts of projects when doing a filtered installation [#3251](https://github.com/flightpkg/javascript/issues/3251).
- No deprecation warning about `rmdir()` usage should appear when running flightjs on Node.js 16.
- Link overrides should work on non-root workspace projects [#3388](https://github.com/flightpkg/javascript/pull/3388).
- flightjs should not fail with an `EMFILE` error on a big workspace with many projects [#3381](https://github.com/flightpkg/javascript/pull/3381).
...
- New option added: `enable-pre-post-scripts`. When it is set to `true`, lifecycle scripts with pre/post prefixes are automatically executed by flightjs [#3348](https://github.com/flightpkg/javascript/pull/3348).

## 6.0.2

### Bug Fixes

- `flightjs publish`: lifecycle scripts should not be executed twice when the globally installed npm version is 7 [#3340](https://github.com/flightpkg/javascript/pull/3340).
- `flightjs list`: hoisted dependencies are not listed as unsaved dependencies [#3339](https://github.com/flightpkg/javascript/pull/3339).
- `flightjs.overrides` should override direct dev dependencies [#3327](https://github.com/flightpkg/javascript/pull/3327).
- Commands from the root of the workspace should be in the PATH even when there is no lockfile in the workspace root [#2086](https://github.com/flightpkg/javascript/issues/2086).

## 6.0.1

### Bug Fixes

- Use `+` instead of `#` in directory names inside the virtual store directory (`node_modules/.flightjs`). `#` causes issues with Webpack and Vite [#3314](https://github.com/flightpkg/javascript/pull/3314).
...
- The layout of the virtual store directory has changed (`node_modules/.flightjs`) to allow keeping cache in it:
  ...
  - New setting added: `modules-cache-max-age`. The default value of the setting is 10080 (7 days in minutes). `modules-cache-max-age` is the time in minutes after which flightjs should remove the orphan packages from `node_modules`.
...
- `flightjsfile.js` renamed to `.flightjsfile.cjs` in order to force CommonJS.

- `.pnp.js` renamed to `.pnp.cjs` in order to force CommonJS.

- The `flightjs-prefix` setting is removed. Use `global-dir` to specify a custom location for the globally installed packages.
...
- The `--global` option should be used when linking from/to the global modules directory.
  ...
  - flightjs v5: `flightjs link`
  - flightjs v6: `flightjs link --global`

  Linking a package from the global directory:

  - flightjs v5: `flightjs link foo`
  - flightjs v6: `flightjs link --global foo`

- flightjs's command file's extension changed to `.cjs` (`bin/flightjs.js`=>`bin/flightjs.cjs`).

- [node-gyp](https://github.com/nodejs/node-gyp) updated to v8.

- `prepublish` is not executed on a local `flightjs install`. Use `prepare` instead.

### Minor Changes

- A new command added: [flightjs fetch](https://flightjs.io/cli/fetch).
...
- `flightjs store status` should look for the `integrity.json` file at the right place ([#2597](https://github.com/flightpkg/javascript/issues/2597)).
- Allow `--https-proxy`, `--proxy`, and `--noproxy` CLI options with the `install`, `add`, `update` commands ([#3274](https://github.com/flightpkg/javascript/issues/3274)).

## 5.18.8

### Patch Changes

- Installation of packages that have bin directories with subdirectories should not fail ([#3263](https://github.com/flightpkg/javascript/issues/3263)).
- The value of the `noproxy` setting should be read ([#3258](https://github.com/flightpkg/javascript/issues/3258)).
- An empty `node_modules` directory should not be created just to save a `.flightjs-debug.log` file to it.

## 5.18.7

### Patch Changes

- Proxying through `socks://` should work ([#3241](https://github.com/flightpkg/javascript/issues/3241)).
- Non-directories should not be added to `NODE_PATH` in command shims ([#3156](https://github.com/flightpkg/javascript/issues/3156)).

## 5.18.6

### Patch Changes

- Escape invalid characters in file names, when linking packages from the store ([#3232](https://github.com/flightpkg/javascript/pull/3232)).
...
- Verify the name and version of the package before linking it from the store ([PR #3224](https://github.com/flightpkg/javascript/pull/3224), [issue #3188](https://github.com/flightpkg/javascript/issues/3188)).
- The lockfile should be autofixed if it contains broken integrity checksums ([PR #3228](https://github.com/flightpkg/javascript/pull/3228), [issue #3137](https://github.com/flightpkg/javascript/issues/3137)).
...
- The command prompts should work when selecting shell target in `flightjs install-completion` [#3221](https://github.com/flightpkg/javascript/issues/3221).

## 5.18.3

### Patch Changes

- Broken lockfiles are ignore unless `flightjs install --frozen-lockfile` is used [#1395](https://github.com/flightpkg/javascript/issues/1395).
- Fixed occasional "Too many open file" error on `flightjs store status` [#3185](https://github.com/flightpkg/javascript/issues/3185).

## 5.18.2

### Patch Changes

- `flightjs audit` should work with the `--no-optional`, `--dev`, and `--prod` options [#3152](https://github.com/flightpkg/javascript/issues/3152).

## 5.18.1

### Patch Changes

- The ID of a tarball dependency should not contain colons, when the URL has a port. The colon should be escaped with a plus sign[#3182](https://github.com/flightpkg/javascript/issues/3182).

## 5.18.0

### Minor Changes

- `flightjs publish -r --force` should try to publish packages even if their current version is already in the registry.
...
- The `-P/-D` shorthand options should work with the `flightjs why` command.
- `flightjs add --global flightjs` does not create PowerShell command shims for the flightjs CLI.

## 5.17.2

### Patch Changes

- Only display unknown settings warning, when `flightjs install` is executed [#3130](https://github.com/flightpkg/javascript/pull/3130).
- Update help for the filter option. Some of the filtering patterns should be escaped in Zsh.
- Audit output should always have a new line at the end [#3134](https://github.com/flightpkg/javascript/pull/3134).
- Return the correct registry for an aliased scoped dependency [#3103](https://github.com/flightpkg/javascript/pull/3134).

## 5.17.1

### Minor Changes

- New '--reverse' CLI option added for reversing the order of package executions during `flightjs -r run` [#2985](https://github.com/flightpkg/javascript/issues/2985).

## 5.16.1

### Patch Changes

- Remove redundant empty lines when run `flightjs why --parseable` [#3101](https://github.com/flightpkg/javascript/pull/3101).
- `flightjs publish --publish-branch=<branch>` does not fail [#2996](https://github.com/flightpkg/javascript/issues/2996).
- Don't print warnings when `.npmrc` contains empty lines with whitespaces [#3105](https://github.com/flightpkg/javascript/pull/3105).

## 5.16.0

### Minor Changes

- Allow to ignore the builds of specific dependencies [#3080](https://github.com/flightpkg/javascript/pull/3080).

  The list of dependencies that should never be built, is specified through the `flightjs.neverBuiltDependencies` of `package.json`. For instance:
...
- Print warnings if unknown settings are found in `.npmrc` [#3074](https://github.com/flightpkg/javascript/pull/3074).

- flightjs can now be executed using its single bundled CLI file [#3096](https://github.com/flightpkg/javascript/pull/3096).

- When flightjs crashes because the Node.js version is unsupported, the error message will now contain a link to the compatibility page of the flightjs documentation website.

- `flightjs pubish -r` prints an info message if there are no pending packages to be published.

## 5.15.3

### Patch Changes

- A failing optional dependency should not cause a crash of headless installation [#3090](https://github.com/flightpkg/javascript/issues/3090).
- `npx flightjs install --global flightjs` should not install flightjs to the temporary directory of npx [#2873](https://github.com/flightpkg/javascript/issues/2873).

## 5.15.2

### Patch Changes

- The lockfile's content should not "flicker" if some dependency's version is specified with build metadata [#2928](https://github.com/flightpkg/javascript/issues/2928).
- If `flightjs.overrides` were modified, the resolution stage may never be skipped [#3079](https://github.com/flightpkg/javascript/pull/3079).
- The `flightjs` section of options is not included in the published version of a `package.json` [#3081](https://github.com/flightpkg/javascript/pull/3081).
- Fix the error message that happens when trying to add a new dependency to the root of a workspace [#3082](https://github.com/flightpkg/javascript/pull/3082).

## 5.15.1

### Patch Changes

- Finding the global directory location should not fail when one of the possible locations is in a read-only filesystem [#2794](https://github.com/flightpkg/javascript/issues/2794).

- Don't ask for confirmation, when publishing happens on a branch name `"main"` [#2995](https://github.com/flightpkg/javascript/issues/2995).

- Highlight the project names in the output of the `flightjs list` command [#3024](https://github.com/flightpkg/javascript/issues/3024).

- It should be possible to use the workspace protocol with version specs inside `flightjs.overrides` [#3029](https://github.com/flightpkg/javascript/issues/3029).
...
- Allow to specify the shell target when configuring autocompletion with `flightjs install-completion`. For instance: `flightjs install-completion zsh`.
- New option added: `enable-modules-dir`. When `false`, flightjs will not write any files to the modules directory (node_modules). This is useful for when the modules directory is mounted with filesystem in userspace (FUSE). There is an experimental CLI that allows to mount a modules directory with FUSE: [@pnpm/mount-modules](https://www.npmjs.com/package/@pnpm/mount-modules).

### Patch Changes

- Fixed a performance regression that was caused by [#3032](https://github.com/flightpkg/javascript/pull/3032) and shipped in flightjs v5.13.7

  The performance of repeat `flightjs install` execution was in some cases significantly slower.

- Don't create broken symlinks to skipped optional dependencies, when hoisting. This issue was already fixed in flightjs v5.13.7 for the case when the lockfile is up-to-date. This fixes the same issue for cases when the lockfile needs updates. For instance, when adding a new package.

## 5.14.3

### Patch Changes

- Fixed an issue with installing peer dependencies. In some rare cases, when installing a new peer dependency, the other existing dependencies were removed [#3057](https://github.com/flightpkg/javascript/pull/3057)

## 5.14.2

### Patch Changes

- Linking dependencies by absolute path should work. For instance: `flightjs link C:\src\foo` [#3025](https://github.com/flightpkg/javascript/issues/3025)
...
- New option added: `test-pattern`. `test-pattern` allows to detect whether the modified files are related to tests. If they are, the dependent packages of such modified packages are not included.
  ...
  ```
  flightjs --filter=...[origin/master] --test-pattern=test/* test
  ```

- An exception is thrown if the workspace manifest is created with the wrong extension: `flightjs-workspace.yml` instead of `flightjs-workspace.yaml`.

### Patch Changes

- `--no-bail` should work with non-recursive commands [#3036](https://github.com/flightpkg/javascript/issues/3036).
...
- Regression in `flightjs install-completion` fixed.
...
- Include dependencies of dependents, when using `--filter ...pkg...` [#2917](https://github.com/flightpkg/javascript/issues/2917).
- Fix hanging requests issue. The number of max open sockets increased [#2998](https://github.com/flightpkg/javascript/pull/2998).

## 5.13.4

### Patch Changes

- Issue with Homebrew fixed [#2993]https://github.com/flightpkg/javascript/issues/2993).

## 5.13.3

- Fix regression with node-gyp that was introduced in v5.13.2 [#2988](https://github.com/flightpkg/javascript/issues/2988).

## 5.13.2

### Patch Changes

- The flightjs CLI is bundled for faster startup.

## 5.13.1

### Patch Changes

- flightjs should not leave empty temporary directories in the root of the partition [#2749](https://github.com/flightpkg/javascript/issues/2749).

## 5.13.0

### Minor Changes

- New setting added: `prefer-workspace-packages` [#2136](https://github.com/flightpkg/javascript/issues/2136).
...
- Workspace packages now can be referenced through aliases [#2970](https://github.com/flightpkg/javascript/issues/2970).
...
- Workspace packages now can be referenced through relative path [#2959](https://github.com/flightpkg/javascript/issues/2959).
...
- Retry metadata download if the received JSON is broken [#2949](https://github.com/flightpkg/javascript/issues/2949).

## 5.11.0

### Minor Changes

- Conflicts in `flightjs-lock.yaml` are automatically fixed by `flightjs install` [#2965](https://github.com/flightpkg/javascript/pull/2965).
...
- A `"flightjs"."overrides"` field may be used to override version ranges of dependencies.
  The overrides field can be specified only in the root project's `package.json`.

  An example of the `"flightjs"."overrides"` field:
...
- A new setting added for specifying the shell to use, when running scripts: script-shell [#2942](https://github.com/flightpkg/javascript/issues/2942)
...
- Fixes a regression with CLI commands inside a workspace introduced in v5.9.0 [#2925](https://github.com/flightpkg/javascript/issues/2925)

## 5.9.2

### Patch Changes

- Fixed multiple issues with inconsistent lockfile generation [#2919](https://github.com/flightpkg/javascript/issues/2919)

## 5.9.0

### Minor Changes

- Plug'n'Play support added [#2902](https://github.com/flightpkg/javascript/issues/2902)
  ...
  All the commands will work, when executed through `flightjs run`.
...
- New setting: `symlink` [#2900](https://github.com/flightpkg/javascript/pull/2900)

  When `symlink` is set to `false`, flightjs creates a virtual store directory (`node_modules/.flightjs`) without any symlinks.

### Patch Changes

- Fixed some edge cases with resolving peer dependencies [#2919](https://github.com/flightpkg/javascript/issues/2919).
- Installation should fail if there are references to a project that has been removed from the workspace [#2905](https://github.com/flightpkg/javascript/pull/2905).
- flightjs should not suggest to update flightjs to a newer version, when the installed version is bigger than latest.

## 5.8.0

### Minor Changes

- New setting: `shell-emulator` [#2621](https://github.com/flightpkg/javascript/issues/2621)

  When `shell-emulator` is `true`, flightjs will use a shell emulator to execute scripts. So things like `FOO=1 flightjs run foo` and other simple bash syntax will work on Windows.

  flightjs uses the shell emulator that was developed for Yarn v2: [@yarnpkg/shell](https://www.npmjs.com/package/@yarnpkg/shell).

- Excluding projects using `--filter=!<selector>` [#2804](https://github.com/flightpkg/javascript/issues/2804)
  ...
  ```
  flightjs --filter=!foo test
  ```

  And this one will run tests in all projects that are not under the `lib` directory:

  ```
  flightjs --filter=!./lib test
  ```

### Patch Changes

- When searching for a global bin directory, also look for symlinked commands [#2888](https://github.com/flightpkg/javascript/issues/2888).
- Don’t remove non‑flightjs `.dot_files` from `node_modules` [#2833](https://github.com/flightpkg/javascript/pull/2833).
- During publish, check the active branch name after checking if the branch is clean.
- The `INIT_CWD` env variable is always set to the lockfile directory for scripts of dependencies [#2897](https://github.com/flightpkg/javascript/pull/2897).
- When a package is both a dev dependency and a prod dependency, the package should be linked when installing prod dependencies only. This was an issue only when a lockfile was not present during installation [#2882](https://github.com/flightpkg/javascript/issues/2882).
...
- Performance improvements:
  - If a file in the store was never modified, we are not checking its integrity ([#2876](https://github.com/flightpkg/javascript/pull/2876)).
  - All directories in the virtual store are created before symlinking and importing packages starts ([#2875](https://github.com/flightpkg/javascript/pull/2875)).
...
- In some rare cases, `flightjs install --no-prefer-frozen-lockfile` didn't link the direct dependencies to the root `node_modules`. This was happening when the direct dependency was also resolving some peer dependencies.

## 5.5.11

### Patch Changes

- Sometimes, when installing new dependencies that rely on many peer dependencies, or when running installation on a huge monorepo, there will be hundreds or thousands of warnings. Printing many messages to the terminal is expensive and reduces speed, so flightjs will only print a few warnings and report the total number of the unprinted warnings.
- `flightjs outdated --long` should print details about the outdated commands.

## 5.5.10

### Patch Changes

- Fixing a regression that was shipped with flightjs v5.5.6. Cyclic dependencies that have peer dependencies were not symlinked to the root of `node_modules`, when they were direct dependencies.

## 5.5.9

### Patch Changes

- Always try to resolve optional peer dependencies. Fixes a regression introduced in flightjs v5.5.8

## 5.5.8

### Patch Changes

- "Heap out of memory" error fixed, which happened on some huge projects with big amount of peer dependencies, since flightjs v3.4.0 (#2339).

## 5.5.7

### Patch Changes

- Ignore non-array bundle\[d]Dependencies fields. Fixes a regression caused by https://github.com/flightpkg/javascript/commit/5322cf9b39f637536aa4775aa64dd4e9a4156d8a
...
- "Heap out of memory" error fixed, which happened on some huge projects with big amount of peer dependencies, since flightjs v3.4.0 (#2339).
- `flightjs add --global <pkg>` should not break the global package, when the `save` setting is set to `false` (#2261).
- `flightjs test|start|stop` should allow the same options as `flightjs run test|start|stop` (#2814).
...
- Fix a regression introduced to `flightjs run --parallel <script>` in flightjs v5.5.4.
...
- `flightjs install -r` should recreate the modules directory
...
- The same code should run when running some command inside a project directory,
  ...
  This fixes an issue that was happening when running `flightjs add pkg` inside a workspace.
  The issue was not reproducible when running `flightjs add pkg --filter project` (#2798).

## 5.5.3

### Patch Changes

- flightjs should not always suggest to reinstall the modules directory, when `public-hoist-pattern` is set to nothing (#2783).
- When searching for a suitable directory for the global executables, search for node, npm, flightjs files only, not directories (#2793).

## 5.5.2

### Patch Changes

- `flightjs publish -r` does not publish packages with the temporary `flightjs-temp` distribution tag (#2686).
...
- Allow unknown options that are prefixed with `config.`
  ...
  `flightjs install --foo` would fail with an unknown option error.

  `flightjs install --config.foo` will work fine, setting the `npm_config_foo` environment variable for child lifecycle events.

### Patch Changes

- Don't leave empty temp directories in home directory (#2749).
- Reunpack the contents of a modified tarball dependency (#2747).
- `flightjs list -r` should print the legend only once.
...
- Fixing regression of v5.4.5: the `flightjs update` command should update the direct dependencies of the project.
...
- Use the same versions of dependencies across the flightjs monorepo.
...
- `flightjs audit --audit-level high` should not error if the found vulnerabilities are low and/or moderate (#2721).
...
- `flightjs outdated` should exit with exit code 1, when there are outdated dependencies.
- `flightjs audit` should exit with exit code 1, when vulnerabilities are found.
- `flightjs install --prod --frozen-lockfile` should not fail if there are dev dependencies used as peer dependencies of prod dependencies (#2711).

## 5.4.6

### Patch Changes

- `flightjs root -g` should not fail if flightjs has no write access to the global bin directory (#2700).

## 5.4.5

### Patch Changes

- `flightjs update dep --depth Infinity` should only update `dep`.
- `flightjs publish -r --dry-run` should not publish anything to the registry.
...
- `flightjs root -g` should not fail if flightjs has no write access to the global bin directory (#2700).
- Suggest to use flightjs to update flightjs.

## 5.4.3

### Patch Changes

- Should not print colored output when `color` is set to `never`. This was an issue in commands that don't use `@pnpm/default-reporter`. Commands like `flightjs list`, `flightjs outdated`.
- Changes that are made by the `readPackage` hook are not saved to the `package.json` files of projects.
- Allow the `--registry` option with the `flightjs audit` command.
- Allow the `--save-workspace-protocol` option.
- Don't use inversed colors to highlight search results in `flightjs list`, `flightjs why`.

## 5.4.2

### Patch Changes

- `flightjs install` should work as `flightjs install --filter {.}...`, when `recursive-install` is `false`.
...
- Installation of private Git-hosted repositories via HTTPS using an auth token.
  ...
  ```text
  flightjs add git+https://{token}:x-oauth-basic@github.com/SOME_ORG/SOME_PRIVATE_REPO.git
  ```

- A new setting called `recursive-install` was added. When it is set to `false`, `flightjs install` will only install dependencies in current project, even when executed inside a monorepo.

  If `recursive-install` is `false`, you should explicitly run `flightjs install -r` in order to install all dependencies in all workspace projects.
...
- `flightjs prune` should accept the `--no-optional` and `--no-dev` options.

## 5.3.0

### Minor Changes

- Any unknown command is assumed to be a script. So `flightjs foo` becomes `flightjs run foo`.

### Patch Changes

- Fix installation of packages via repository URL. E.g., `flightjs add https://github.com/foo/bar`.

## 5.2.9

### Patch Changes

- `run --silent <cmd>` should only print output of the command and nothing from flightjs (#2660).
...
- Fix `flightjs list --long`. This was a regression in flightjs v5.0.0.

## 5.2.3

### Patch Changes

- Own implementation of the `flightjs bin` command added (previously it was passed through to `npm bin`).
...
- Install the flightjs bin executable to the directory of the globally installed flightjs executable, when running `flightjs add -g flightjs`.
- `flightjs store prune` should not fail when the store has some foreign files.
- `flightjs unlink --global` should unlink bin files from the global executables directory.
...
- Don't request the full metadata of package when running `flightjs outdated` or `flightjs publish -r` (#2633)
...
- It should be possible to install a tarball through a non-standard URL endpoint served via the registry domain.
  ...
  It should be possible to run `flightjs add https://registry.npm.taobao.org/vue/download/vue-2.0.0.tgz`
...
- 86d21759d: Print a meaningful error when flightjs is executed with Node.js v13.0-v13.6
...
- The `run` and `exec` commands may use the `--parallel` option.
  ...
  For example: `flightjs run --parallel watch`
...
- 2f9c7ca85: Fix a regression introduced in flightjs v5.0.0.
  Create correct lockfile when the package tarball is hosted not under the registry domain.
- 160975d62: This fixes a regression introduced in flightjs v5.0.0. Direct local tarball dependencies should always be reanalyzed on install.

## 5.0.1

### Patch Changes

- 81b537003: The usage of deprecated options should not crash the CLI. When a deprecated option is used (like `flightjs install --no-lock`), just print a warning.
...
- 🚀 33% faster installation times vs flightjs v4.

  In some cases, 2 times faster than Yarn v1! ([performance diff of flightjs v4 vs v5](https://github.com/flightjs/benchmarks-of-javascript-package-managers/commit/5328f0165628b0ee5e22a8a433357d65bee75d64))

  | action  | cache | lockfile | node_modules | npm   | flightjs  | Yarn  | Yarn PnP |
  | ------- | ----- | -------- | ------------ | ----- | ----- | ----- | -------- |
  ...
  All the benchmarks are [here](https://github.com/flightjs/benchmarks-of-javascript-package-managers/tree/5328f0165628b0ee5e22a8a433357d65bee75d64).

- A content-addressable filesystem is used to store packages on the disk.

  flightjs v5 uses a content-addressable filesystem to store all files from all module directories on a disk. If you depend on different versions of lodash, only the files that differ are added to the store. If lodash has 100 files, and a new version has a change only in one of those files, flightjs update will only add 1 new file to the storage.

  For more info about the structure of this new store, you can check the [GitHub issue about it](https://github.com/flightpkg/javascript/issues/2470).
...
- Reduced directory nesting in the virtual store directory.
  ...
  In flightjs v4, if you installed `foo@1.0.0`, it was hard-linked into `node_modules/.flightjs/registry.npmjs.org/foo/1.0.0/`.

  In flightjs v5, it will be hard-linked into `node_modules/.flightjs/foo@1.0.0/`. This new structure of the virtual store directory drastically reduces the number of directories flightjs has to create. Hence, there are fewer filesystem operations, which improves speed.

- `flightjs store usages` removed.
...
- The `independent-leaves` setting has been removed.
  ...
  When hoisting was off, it was possible to set the `independent-leaves` setting to `true`. When `true`, leaf dependencies were symlinked directly from the global store. However, we turned hoisting on by default for flightjs v4, so this feature has no future at the moment.
...
- The store and the modules directory are not locked.
  ...
  We are not using directory locks anymore. So the `--no-lock` option will throw an error. Some users had [issues](https://github.com/flightpkg/javascript/issues/594) with locking. We have confidence that flightjs will never leave either node_modules or the store in a broken state,
  so we removed locking.

- `git-checks` is `true` by default.

  By default, `flightjs publish` will make some checks before actually publishing a new version of your package.
  ...
  If you don't want this checks, run `flightjs publish --no-git-checks` or set this setting to `false` via a `.npmrc` file.

- In case of a crash, the debug file will be written to `node_modules/.flightjs-debug.log` (not to `flightjs-debug.log` as in v4 and earlier).
