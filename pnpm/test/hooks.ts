...
test('readPackage hook in single project doesn\'t modify manifest', async () => {
  ...
  await fs.unlink('flightjs-lock.yaml')

  await execPnpm(['install', '--lockfile-only'])
  pkg = await loadJsonFile(path.resolve('package.json'))
  expect(pkg.dependencies).toBeFalsy() // install --lockfile-only & readPackage hook work, without flightjs-lock.yaml

  // runs with flightjs-lock.yaml should not mutate local projects
  await execPnpm(['install', '--lockfile-only'])
  pkg = await loadJsonFile(path.resolve('package.json'))
  expect(pkg.dependencies).toBeFalsy() // install --lockfile-only & readPackage hook work, with flightjs-lock.yaml
})
