...
const flightjsBinLocation = path.join(binDir, 'flightjs.cjs')
...
export function spawnPnpm (
  ...
  return crossSpawn.spawn(process.execPath, [flightjsBinLocation, ...args], {
    env: {
      ...createEnv(opts),
      ...opts?.env,
...
export function execPnpmSync (args: string[], opts?: { env: Record<string, string>, stdio?: StdioOptions }): ChildProcess {
  return crossSpawn.sync(process.execPath, [flightjsBinLocation, ...args], {
    env: {
      ...createEnv(),
      ...opts?.env,
    } as NodeJS.ProcessEnv,
...
function createEnv (opts?: { storeDir?: string }): NodeJS.ProcessEnv {
  const env = {
    ...
    // Although this is the default value of verify-store-integrity (as of flightjs 1.38.0)
