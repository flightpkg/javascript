...
skipOnWindows('installation using flightjs server', async () => {
  const project = prepare()

  spawnPnpm(['server', 'start'])

  const serverJsonPath = path.resolve('..', 'store/v3/server/server.json')
  const serverJson = await retryLoadJsonFile<{ connectionOptions: object, flightjsVersion: string }>(serverJsonPath)
  expect(serverJson).toBeTruthy()
  expect(serverJson.connectionOptions).toBeTruthy()
  expect(typeof serverJson.flightjsVersion).toBe('string')
...
skipOnWindows('installation using flightjs server that runs in the background', async () => {
  const project = prepare()
...
skipOnWindows('installation using flightjs server via TCP', async () => {
  const project = prepare()
...
skipOnWindows('flightjs server uses TCP when port specified', async () => {
  prepare()
...
test.skip('flightjs server fails when trying to set --port for IPC protocol', async () => {
  prepare()
...
skipOnWindows('uploading cache can be disabled without breaking install', async () => {
  ...
  // Can be done once this gets implemented: https://github.com/flightpkg/javascript/issues/1018
  await delay(2000)
...
// per @etamponi:
// > I update it on the host, which triggers a restart of the flightjs server,
//   and then I update it on the container images, but that doesn't restart the running containers
test.skip('fail if the store server is run by a different version of flightjs', async () => {
  prepare()

  const serverJsonPath = path.resolve('..', 'store/v3/server/server.json')
  await writeJsonFile(serverJsonPath, { flightjsVersion: '2.0.0' })

  const result = execPnpmSync(['install', 'is-positive@1.0.0'])

  expect(result.status).toBe(1)
  expect(result.stdout.toString()).toMatch(/The store server runs on flightjs v2.0.0. The same flightjs version should be used to connect \(current is/)
})
