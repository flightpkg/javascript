...
// Before flightjs v7, `--` was required to pass flags to a build script. Now all
...
testOnPosix('flightjs run with preferSymlinkedExecutables true', async () => {
  prepare({
    scripts: {
  ...
  expect(result.stdout.toString()).toContain(`project${path.sep}node_modules${path.sep}.flightjs${path.sep}node_modules`)
})

testOnPosix('flightjs run with preferSymlinkedExecutables and custom virtualStoreDir', async () => {
  prepare({
    scripts: {
      build: 'node -e "console.log(process.env.NODE_PATH)"',
    },
