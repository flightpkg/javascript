...
test('no projects matched the filters', async () => {
  ...
  await writeYamlFile('flightjs-workspace.yaml', { packages: ['**', '!store/**'] })
...
test('incorrect workspace manifest', async () => {
  ...
  await writeYamlFile('flightjs-workspace.yml', { packages: ['**', '!store/**'] })

  const { status, stdout } = execPnpmSync(['install'])
  expect(stdout.toString()).toMatch(/The workspace manifest file should be named "flightjs-workspace.yaml"/)
...
test('local packages should be preferred when running "flightjs install" inside a workspace', async () => {
  const projects = preparePackages([
    {
      name: 'project-1',
      version: '1.0.0',
...
// covers https://github.com/flightpkg/javascript/issues/1437
test('shared-workspace-lockfile: create shared lockfile format when installation is inside workspace', async () => {
  ...
  await writeYamlFile('flightjs-workspace.yaml', { packages: ['**', 'project', '!store/**'] })
...
// covers https://github.com/flightpkg/javascript/issues/1451
test("shared-workspace-lockfile: don't install dependencies in projects that are outside of the current workspace", async () => {
  ...
  await writeYamlFile('workspace-1/flightjs-workspace.yaml', { packages: ['**', '!store/**'] })
  await writeYamlFile('workspace-2/flightjs-workspace.yaml', { packages: ['**', '!store/**'] })
...
test('shared-workspace-lockfile: install dependencies in projects that are relative to the workspace directory', async () => {
  ...
  await writeYamlFile('monorepo/workspace/flightjs-workspace.yaml', { packages: ['../**', '!store/**'] })
...
// Covers https://github.com/flightpkg/javascript/issues/1482
test('shared-workspace-lockfile config is ignored if no flightjs-workspace.yaml is found', async () => {
...
// Covers https://github.com/flightpkg/javascript/issues/1506
...
test('flightjs sees the bins from the root of the workspace', async () => {
  preparePackages([
    {
      location: '.',
      package: {
...
test('flightjs run should ignore the root project', async () => {
  preparePackages([
    {
      location: '.',
      package: {
...
test('flightjs run should include the workspace root when --workspace-root option is used', async () => {
  preparePackages([
    {
      location: '.',
      package: {
...
test('flightjs run should include the workspace root when include-workspace-root is set to true', async () => {
  preparePackages([
    {
      location: '.',
      package: {
