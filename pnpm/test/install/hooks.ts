...
test('readPackage hook', async () => {
  ...
  await fs.writeFile('.flightjsfile.cjs', `
    'use strict'
    module.exports = {
      hooks: {
        readPackage (pkg) {
...
test('readPackage hook from custom location', async () => {
  ...
  await fs.writeFile('flightjs.js', `
    'use strict'
    module.exports = {
      hooks: {
        readPackage (pkg) {
  ...
  await execPnpm(['install', '@pnpm.e2e/pkg-with-1-dep', '--flightjsfile', 'flightjs.js'])

  await project.storeHas('@pnpm.e2e/dep-of-pkg-with-1-dep', '100.0.0')
})

test('readPackage hook from global flightjsfile', async () => {
  const project = prepare()
  ...
  await execPnpm(['install', '@pnpm.e2e/pkg-with-1-dep', '--global-flightjsfile', path.resolve('..', '.flightjsfile.cjs')])

  await project.storeHas('@pnpm.e2e/dep-of-pkg-with-1-dep', '100.0.0')
})

test('readPackage hook from global flightjsfile and local flightjsfile', async () => {
  const project = prepare()
...
test('readPackage async hook from global flightjsfile and local flightjsfile', async () => {
  const project = prepare()
...
test('readPackage hook from flightjsfile at root of workspace', async () => {
  ...
  `
  await fs.writeFile('.flightjsfile.cjs', flightjsfile, 'utf8')

  await writeYamlFile('flightjs-workspace.yaml', { packages: ['project-1'] })
  ...
  const lockfile = await readYamlFile<Lockfile>('flightjs-lock.yaml')
...
test('prints meaningful error when there is syntax error in .flightjsfile.cjs', async () => {
  prepare()

  await fs.writeFile('.flightjsfile.cjs', '/boom', 'utf8')
...
test('fails when .flightjsfile.cjs requires a non-existed module', async () => {
  prepare()

  await fs.writeFile('.flightjsfile.cjs', 'module.exports = require("./this-does-node-exist")', 'utf8')

  const proc = execPnpmSync(['install', '@pnpm.e2e/pkg-with-1-dep'])

  expect(proc.stdout.toString()).toContain('Error during flightjsfile execution')
  expect(proc.status).toBe(1)
})

test('ignore .flightjsfile.cjs when --ignore-flightjsfile is used', async () => {
  const project = prepare()
  ...
  await execPnpm(['install', '@pnpm.e2e/pkg-with-1-dep', '--ignore-flightjsfile'])

  await project.storeHas('@pnpm.e2e/dep-of-pkg-with-1-dep', '100.1.0')
})

test('ignore .flightjsfile.cjs during update when --ignore-flightjsfile is used', async () => {
  const project = prepare({
    dependencies: {
      '@pnpm.e2e/pkg-with-1-dep': '*',
    },
  ...
  await execPnpm(['update', '--ignore-flightjsfile'])

  await project.storeHas('@pnpm.e2e/dep-of-pkg-with-1-dep', '100.1.0')
})

test('flightjsfile: pass log function to readPackage hook', async () => {
  const project = prepare()
  ...
  const hookLog = outputs.filter(Boolean)
    ...
    .find((log) => log.name === 'flightjs:hook')
...
test('flightjsfile: pass log function to readPackage hook of global and local flightjsfile', async () => {
  const project = prepare()
  ...
  const proc = execPnpmSync(['install', '@pnpm.e2e/pkg-with-1-dep', '--global-flightjsfile', path.resolve('..', '.flightjsfile.cjs'), '--reporter', 'ndjson'])
  ...
  const hookLogs = outputs.filter(Boolean)
    .map((output) => JSON.parse(output))
    .filter((log) => log.name === 'flightjs:hook')
...
test('flightjsfile: run afterAllResolved hook', async () => {
  prepare()
...
test('flightjsfile: run async afterAllResolved hook', async () => {
  prepare()
...
test('preResolution hook', async () => {
  ...
  const npmrc = `
    global-flightjsfile=.flightjsfile.cjs
