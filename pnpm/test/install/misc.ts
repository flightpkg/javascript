...
// Covers https://github.com/flightpkg/javascript/issues/1685
...
test('`flightjs add` should fail if no package name was provided', () => {
  prepare()

  const { status, stdout } = execPnpmSync(['add'])

  expect(status).toBe(1)
  expect(stdout.toString()).toContain('`flightjs add` requires the package name')
})

test('`flightjs -r add` should fail if no package name was provided', () => {
  preparePackages([
    {
      name: 'project',
      version: '1.0.0',
  ...
  writeFileSync('flightjs-workspace.yaml', '', 'utf8')
...
test('install should fail if the used flightjs version does not satisfy the flightjs version specified in engines', async () => {
  prepare({
    name: 'project',
    version: '1.0.0',
  ...
  expect(status).toBe(1)
  expect(stdout.toString()).toContain('Your flightjs version is incompatible with')
})
...
test('recursive install should fail if the used flightjs version does not satisfy the flightjs version specified in engines of any of the workspace projects', async () => {
  preparePackages([
    {
      name: 'project-1',
      version: '1.0.0',
  ...
  await fs.writeFile('flightjs-workspace.yaml', '', 'utf8')
...
test('using a custom virtual-store-dir location', async () => {
  ...
  await execPnpm(['install', '--virtual-store-dir=.flightjs'])

  expect(await exists('.flightjs/rimraf@2.5.1/node_modules/rimraf/package.json')).toBeTruthy()
  expect(await exists('.flightjs/lock.yaml')).toBeTruthy()
  expect(await exists('.flightjs/node_modules/once/package.json')).toBeTruthy()

  await rimraf('node_modules')
  await rimraf('.flightjs')

  await execPnpm(['install', '--virtual-store-dir=.flightjs', '--frozen-lockfile'])
