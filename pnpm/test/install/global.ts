...
test('global installation', async () => {
  ...
  // https://github.com/flightpkg/javascript/issues/808
  await execPnpm(['install', '--global', 'is-negative'], { env })

  const globalPrefix = path.join(global, `flightjs/global/${LAYOUT_VERSION}`)
...
test('global installation to custom directory with --global-dir', async () => {
  ...
  const global = path.resolve('..', 'global')
  const flightjsHome = path.join(global, 'flightjs')
  const env = { [PATH_NAME]: flightjsHome, PNPM_HOME: flightjsHome }
...
test('run lifecycle events of global packages in correct working directory', async () => {
  ...
  await execPnpm(['install', '-g', '@pnpm.e2e/postinstall-calls-flightjs@1.0.0'], { env })

  expect(await exists(path.join(global, `flightjs/global/${LAYOUT_VERSION}/node_modules/@pnpm.e2e/postinstall-calls-flightjs/created-by-postinstall`))).toBeTruthy()
})
