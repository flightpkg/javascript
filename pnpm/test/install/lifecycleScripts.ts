...
const flightjsPkg = loadJsonFile.sync<PackageManifest>(path.join(pkgRoot, 'package.json'))
...
test('lifecycle script runs with the correct user agent', () => {
  ...
  const expectedUserAgentPrefix = `${flightjsPkg.name}/${flightjsPkg.version} `
...
test('node-gyp is in the PATH', async () => {
  ...
  const result = execPnpmSync(['test'], {
    env: {
      ...
      // it is removed here to test that flightjs adds it
