...
test('hoist the dependency graph', async () => {
  ...
  await project.has('.flightjs/node_modules/debug')
  await project.has('.flightjs/node_modules/cookie')

  await execPnpm(['uninstall', 'express'])

  await project.hasNot('express')
  await project.hasNot('.flightjs/node_modules/debug')
  await project.hasNot('.flightjs/node_modules/cookie')
})
