...
test('recursive installation of packages in workspace ignores hooks in packages', async () => {
  ...
  `
  await fs.writeFile('project-1/.flightjsfile.cjs', flightjsfile, 'utf8')
  await fs.writeFile('project-2/.flightjsfile.cjs', flightjsfile, 'utf8')
  ...
  await writeYamlFile('flightjs-workspace.yaml', { packages: ['project-1', 'project-2'] })
...
test('ignores .flightjsfile.cjs during recursive installation when --ignore-flightjsfile is used', async () => {
  // This test hangs on Appveyor for some reason
  if (isCI && isWindows()) return
  const projects = preparePackages([
    {
  ...
  await execPnpm(['recursive', 'install', '--ignore-flightjsfile'])
