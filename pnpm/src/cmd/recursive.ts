...
A `flightjs-workspace.yaml` file may be used to control what directories are searched for packages.',
    descriptionLists: [
      {
        title: 'Commands',
...
A shared lockfile also means that all dependencies of all projects will be in a single node_modules.`,
    ...
    usages: [
      'flightjs recursive [command] [flags] [--filter <package selector>]',
      'flightjs multi [command] [flags] [--filter <package selector>]',
      'flightjs m [command] [flags] [--filter <package selector>]',
    ],
  })
}
