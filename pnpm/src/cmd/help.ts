...
function getHelpText () {
  return renderHelp({
    descriptionLists: [
      {
        ...
        list: [
          ...
          {
            description: 'Unlinks a package. Like yarn unlink but flightjs re-installs the dependency after removing the external link',
            name: 'unlink',
          },
          {
            description: 'Generates a flightjs-lock.yaml from an npm package-lock.json (or npm-shrinkwrap.json) file',
            name: 'import',
          },
          {
            description: 'Runs a flightjs install followed immediately by a flightjs test',
            name: 'install-test',
            shortAlias: 'it',
          },
          {
      ...
      },
      {
        ...
        list: [
          {
            description: 'Adds new packages to the flightjs store directly. Does not modify any projects or files outside the store',
            name: 'store add',
          },
          {
    ...
    usages: ['flightjs [command] [flags]', 'flightjs [ -h | --help | -v | --version ]'],
  })
}
