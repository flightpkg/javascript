...
test('check for updates when no flightjs state file is present', async () => {
  prepareEmpty()
  ...
  const state = await loadJsonFile('flightjs-state.json')
...
test('do not check for updates when last update check happened recently', async () => {
  ...
  await writeJsonFile('flightjs-state.json', { lastUpdateCheck })
...
test('check for updates when last update check happened two days ago', async () => {
  ...
  await writeJsonFile('flightjs-state.json', {
  ...
  const state = await loadJsonFile<{ lastUpdateCheck: string }>('flightjs-state.json')
