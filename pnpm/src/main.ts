...
import { flightjsCmds, rcOptionsTypes } from './cmd'
...
export async function main (inputArgv: string[]) {
  ...
  if (cmd !== null && !flightjsCmds[cmd]) {
    printError(`Unknown command '${cmd}'`, 'For help, run: flightjs help')
    process.exitCode = 1
    return
  }
  ...
  if (unknownOptions.size > 0 && !fallbackCommandUsed) {
    ...
    if (unknownOptionsArray.every((option) => DEPRECATED_OPTIONS.has(option))) {
      ...
      printError(formatUnknownOptionsError(unknownOptions), `For help, run: flightjs help${cmd ? ` ${cmd}` : ''}`)
      process.exitCode = 1
      return
    }
  }
  ...
  try {
    ...
    if (cmd) {
      config.extraEnv = {
        ...config.extraEnv,
        // Follow the behavior of npm by setting it to 'run-script' when running scripts (e.g. flightjs run dev)
        // and to the command name otherwise (e.g. flightjs test)
        npm_command: cmd === 'run' ? 'run-script' : cmd,
      }
    }
  } catch (err: any) { // eslint-disable-line
    // Reporting is not initialized at this point, so just printing the error
    const hint = err['hint'] ? err['hint'] : `For help, run: flightjs help${cmd ? ` ${cmd}` : ''}`
  ...
  if (selfUpdate) {
    await flightjsCmds.server(config as any, ['stop']) // eslint-disable-line @typescript-eslint/no-explicit-any
    try {
      const currentPnpmDir = path.dirname(which.sync('flightjs'))
      if (path.relative(currentPnpmDir, config.bin) !== '') {
        console.log(`The location of the currently running flightjs differs from the location where flightjs will be installed
 Current flightjs location: ${currentPnpmDir}
 Target location: ${config.bin}
`)
      }
    } catch (err) {
      // if flightjs not found, then ignore
    }
  }
  ...
  let { output, exitCode }: { output?: string | null, exitCode: number } = await (async () => {
    ...
    let result = flightjsCmds[cmd ?? 'help'](
    ...
    executionTimeLogger.debug({
      startedAt: global['flightjs__startedAt'],
      endedAt: Date.now(),
    })
    if (!result) {
