...
test('recursive install on workspace with custom lockfile-dir', async () => {
  ...
  const lockfile = await readYamlFile<Lockfile>(path.join(lockfileDir, 'flightjs-lock.yaml'))
...
test('prefer-workspace-package', async () => {
  ...
  const lockfile = await readYamlFile<Lockfile>(path.resolve('flightjs-lock.yaml'))
